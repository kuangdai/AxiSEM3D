...
Isotropic3X::Isotropic3X(const RDRowN &theta, const RMatXN &lambda, const RMatXN &mu, const RMatXN4 &X, 
    Attenuation3D *att):
Elastic3D(att), mLambda(lambda), mMu(mu), mMu2(two * mMu), mX(X) {
    int Nr = mLambda.rows();
    int n = Nr / 2 + 1;
...
void Isotropic3X::strainToStress(const vec_ar9_CMatPP &strain, vec_ar9_CMatPP &stress, int dummy) const {
    // constants
    int Nr =  mLambda.rows();
    int Nu = Nr / 2;
    ...
    // to avoid dynamic allocation, use stressTIsoR6.block(0, 3 * nPE, Nr, nPE) as temp memory
    stressTIsoR6.block(0, 3 * nPE, Nr, nPE) = mLambda.schur(strainTIsoR6.block(0, 0 * nPE, Nr, nPE) 
                                                          + strainTIsoR6.block(0, 1 * nPE, Nr, nPE) 
                                                          + strainTIsoR6.block(0, 2 * nPE, Nr, nPE));
    stressTIsoR6.block(0, 0 * nPE, Nr, nPE) = stressTIsoR6.block(0, 3 * nPE, Nr, nPE) + mMu2.schur(strainTIsoR6.block(0, 0 * nPE, Nr, nPE));
    stressTIsoR6.block(0, 1 * nPE, Nr, nPE) = stressTIsoR6.block(0, 3 * nPE, Nr, nPE) + mMu2.schur(strainTIsoR6.block(0, 1 * nPE, Nr, nPE));
    stressTIsoR6.block(0, 2 * nPE, Nr, nPE) = stressTIsoR6.block(0, 3 * nPE, Nr, nPE) + mMu2.schur(strainTIsoR6.block(0, 2 * nPE, Nr, nPE));
    stressTIsoR6.block(0, 3 * nPE, Nr, nPE) = mMu.schur(strainTIsoR6.block(0, 3 * nPE, Nr, nPE));
    stressTIsoR6.block(0, 4 * nPE, Nr, nPE) = mMu.schur(strainTIsoR6.block(0, 4 * nPE, Nr, nPE));
    stressTIsoR6.block(0, 5 * nPE, Nr, nPE) = mMu.schur(strainTIsoR6.block(0, 5 * nPE, Nr, nPE));
...
void Isotropic3X::transformStrainX(const RMatXN9 &strain9, RMatXN6 &strain6, int n) const {
...
void Isotropic3X::transformStressX(const RMatXN6 &stress6, RMatXN9 &stress9, int n) const {
...
void Isotropic3X::rotateStrainToTIso(const CMatXN9 &strainCyln, CMatXN9 &strainTIso, int Nu) const {
    int n = Nu + 1;
    // temp
...
void Isotropic3X::rotateStressToCyln(const CMatXN9 &stressTIso, CMatXN9 &stressCyln, int Nu) const
{
    int n = Nu + 1;
    // temp
...
void Isotropic3X::checkCompatibility(int Nr, bool isVoigt) const {
    Elastic3D::checkCompatibility(Nr, isVoigt);
    int myNr = mLambda.rows();
    if (Nr != myNr) 
        throw std::runtime_error("Isotropic3X::checkCompatibility || Incompatible size.");
    if (isVoigt) throw std::runtime_error("Isotropic3X::checkCompatibility || "
