...
module s40rts
    ...
    public :: initialize_s40rts, finalize_s40rts, perturb_s40rts 
    private
...
contains
    ...
    subroutine initialize_s40rts(rcmb, rmoho, rearth, meta_data_p12, meta_data_s40) &
        ...
        do k = 0, NK_20
            do l = 0, NS_40
                S40RTS_V_dvs_a(k, l, 0) = meta_data_s40(pos); pos = pos + 1
                do m = 1, l
                    S40RTS_V_dvs_a(k, l, m) = meta_data_s40(pos); pos = pos + 1
                    S40RTS_V_dvs_b(k, l, m) = meta_data_s40(pos); pos = pos + 1
                enddo
            enddo
        enddo
        ...
        do k = 0, NK_20
            do l = 0, 12
                S40RTS_V_dvp_a(k, l, 0) = meta_data_p12(pos); pos = pos + 1
                do m = 1, l
                    S40RTS_V_dvp_a(k, l, m) = meta_data_p12(pos); pos = pos + 1
                    S40RTS_V_dvp_b(k, l, m) = meta_data_p12(pos); pos = pos + 1
                enddo
            enddo
            do l = 13, NS_40
    ...
    subroutine finalize_s40rts() &
        ...
        if (allocated(S40RTS_V_dvs_a)) then
            deallocate(S40RTS_V_dvs_a, S40RTS_V_dvs_b, & 
            S40RTS_V_dvp_a, S40RTS_V_dvp_b, &
            S40RTS_V_spknt, S40RTS_V_qq0, S40RTS_V_qq)
        endif
        
    end subroutine finalize_s40rts
    ...
    function perturb_s40rts(r_abs, theta, phi, r_center_abs, dvp, dvs) &
        ...
        do k = 0, NK_20
            radial_basis(k) = s40rts_rsple(1, NK_20 + 1, S40RTS_V_spknt(1), &
            S40RTS_V_qq0(1, NK_20 + 1 - k), S40RTS_V_qq(1, 1, NK_20 + 1 - k), xr)
        enddo
    ...
    subroutine s40rts_splhsetup
        ...
        enddo
        do i = 1, NK_20 + 1
            call s40rts_rspln(1, NK_20 + 1, S40RTS_V_spknt(1), &
            S40RTS_V_qq0(1, i), S40RTS_V_qq(1, 1, i), qqwk(1, 1))
        enddo
